...
void solve(){
	...
	vector<int> a(n , 0);
	vector<int> p(n+1 , 0);
	//we want to create this final permutation
	multiset<int> left;
...
/*
	cout<<"PERMUTATION  CURRENTLE"<<endl;
	for(int i=1;i<=n;i++){
		cout<<p[i]<<" ";
	}
	cout<<endl;
	...
	cout<<"LEFt"<<endl;
	for(auto ele : left){cout<<ele<<" ";}cout<<'\n';*/
	ll steps=0LL;
	//now we will check whether building permutation is possible
	for(int ele=1; ele<=n ;ele++){
		if(p[ele] != 0){continue;}
		//pick up minimal
		if(!left.empty() &&   *left.begin() >= ele  ){
			steps++;
			p[ele]=ele;
