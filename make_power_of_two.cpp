...
using namespace std;
/*
use llround when using pow function to avoid precision issue
ll x=llround(pow(10 , 18));
*/
/*
...
int helper(string sn , string sx){
	int i=0;int j=0;
	int taken=0;
	while( (i < sn.size())  &&  (j < sx.size()) ){
		if(sn[i]==sx[j]){
			i++;j++;
			taken++;
		}
...
void solve(){
	...
	//iterate over powers of 2 where x<10^18
...
*/
	...
	while( x < 1e18 ){
		...
		int taken=helper(sn , sx);
		int curr_ans=sn.size()+sx.size()-2*taken;
