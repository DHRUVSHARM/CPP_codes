...
bool helper(vector<vector<int>>& r , int winner , int i){
	int count=0;
	for(int j=0;j<5;j++){
		if(r[i][j] < r[winner][j]){
			count++;
...
//	map<int,int> first;
	...
	int winner=0;
	//assume that the winner is the first player

	for(int i=1;i<n;i++){
		if(helper(r , winner , i)){
			//helper is true means that i is superior
			winner=i;
		}
	}

	//only this winner can be gold qualified if anyone
	for(int i=0;i<n;i++){
		if(i == winner){continue;}
		if(helper(r , winner , i)){
			//winner is not superior
			cout<<"-1"<<'\n';return; 
		}
	}
