...
void solve(){
	...
	for(ll k=1;k<=floor(sqrt(x));k++){
		//this loop already includes the condition that:
		//1<=a<=(x)

		//now find the length of the range of b , if -ve put 0

		special_pairs+=( max(0LL ,  (  min( (x/k) - 1 , y  ) - (k+1) + 1 ) ) );
	}
