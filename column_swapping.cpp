...
void solve(){
	...
	//store all the bad column indices
	vector<int> bad;
	...
	//for(int i=0;i<bad.size();i++){cout<<bad[i]<<" ";}cout<<endl;


	if(bad.empty()){cout<<"1"<<" "<<"1"<<'\n';return;}
	if(bad.size()>2){cout<<"-1"<<'\n';return;}

	//maybe possible to make a swap and make the a as good
	...
	for(int i=0;i<n;i++){
		for(int j=1;j<m;j++){
			if(a[i][j]<a[i][j-1]){cout<<"-1"<<endl;return;}
		}
	}
