...
int binary_search_lowerbound(vector<ll>& a , int n , ll def){
	ll l=-1;ll r=n;
	while( l+1 < r ){
		ll mid = l + (r-l)/2;
		if( a[mid] >= def ){
...
void solve(){
	...
	for(int i=0;i<m;i++){
		...
		int index=binary_search_lowerbound(a , n , defense);

		ll ans=LLONG_MAX;

		if( index < n ){
			ans=min( ans , max( 0LL , ( attack-sum+a[index] ) ) );
		}
		if( index-1 >= 0 ){
			ans=min( ans , defense - a[index-1] + max(0LL , ( attack-sum + a[index-1] )) );
		}
