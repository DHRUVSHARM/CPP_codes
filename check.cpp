...
void dfs(int node , int edgeNumber){
	...
	for(pair<int,int> e : adj[node]){
		if(e.second == edgeNumber) continue;
	...
	if(edgeNumber != 0){
		if(in[node] == low[node]) res[edgeNumber] = 2;
	}
}
...
int main(){
	...
	for(int i=1;i<=m;i++) cin>>edges[i].a>>edges[i].b>>edges[i].w , edges[i].idx = i;
	...
	for(;i<=m;){
		...
		int j;
		for(j=i;edges[i].w == edges[j].w;j++) addEdge(edges[j].a , edges[j].b , edges[j].idx);

		timer = 0;
		for(j=i;edges[i].w == edges[j].w;j++){
			int p = find(edges[j].a);
			if(vis[p] == false) dfs(p , 0);
		}
