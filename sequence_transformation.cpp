...
/*	for(auto p: occ){
	...
	//now iterate over occ of all elements to find answer
	...
	for(auto p:occ){
		...
		if(b[0]>0){curr_deletions++;}if(b[b.size()-1]<(a.size()-1)){curr_deletions++;}
	//	cout<<p.first<<'\t';
	//	cout<<curr_deletions<<'\n';
		deletions=min(curr_deletions , deletions);
	}
