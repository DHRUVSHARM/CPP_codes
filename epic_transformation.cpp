...
void solve(){
	...
	//max priority queue order it by the count of each element
	...
	//note that you cannot iterate over a priority queue
	//can only use pop and access the topmost element

	/*cout<<"topmost"<<'\n';
	auto p=pq.top();
	cout<<p.first<<" "<<p.second<<'\n';*/
	int ans=n; //will give the number of elements left finally
	while(pq.size() >= 2){
		//take the two largest elements
		
		auto p=pq.top();
		int c1=p.first;int ele1=p.second;
		pq.pop();
		p=pq.top();
		int c2=p.first;int ele2=p.second;
		pq.pop();
		c1--;c2--;
		ans-=2;
