...
void solve(){
	...
	vector<int> ten_index;
	for(int i=0;i<(n-1);i++){
		if(x[i]==1 && x[i+1]==0){
			ten_index.push_back(i);
		}
	}

	if(ten_index.size()==0){cout<<s<<'\n';return;}

	int first_ten=ten_index[0];
	int last_ten=ten_index[ten_index.size()-1];

	string ans="";
	for(int i=0;i<n;i++){
		if(i==first_ten){break;}
		if(x[i]==0){ans=ans + '0';}
	}

	ans+='0';

	for(int i=last_ten+2;i<n;i++){
		if(x[i]==1){ans=ans + '1';}
