#include<bits/stdc++.h>
#include<iostream>
using namespace std;
/*
to get unique elements in sorted order
  sort(fun_values.begin(),fun_values.end());
    fun_values.erase(unique(fun_values.begin(),fun_values.end()),fun_values.end());
    for(ll&x:fun_values)
        cout<<x<<" "; 

*/

/*
	vector<int> primelist;
	for(int i=0;i<=x;i++){
		if(seive[i]==1){primelist.push_back(i);}
	}

	vector<int> factors;

	for(int i=0;primelist[i]*primelist[i]<=x && i<primelist.size();i++){
		if(x%primelist[i]==0){
			while(x%primelist[i]==0){
				x/=primelist[i];
				factors.push_back(primelist[i]);
			}
		}
	}
	if(x>1){
	factors.push_back(x);
}

	sort(factors.begin() , factors.end());
*/

/*
int gcd(int a  , int b){
//complexity O(log(min(a , b)))
	while(b){
		a%=b;
		swap(a,  b);
	}
	return a;
}
*/

//move on board
//from up clockwise all 8 directions
/*
int xdif[8]={-1,-1,0,1,1,1,0,-1};
int ydif[8]={0,1,1,1,0,-1,-1,-1};
*/

/*convert integer to string
can convert all data types to string 
string s=to_string(element);*/

/*  stringstream ss;
	string s="3.123";
	//extract the string to ss
	//then it can be put to convert type

	ss<<s;
	float converted_element;
	ss>>converted_element;

	cout<<converted_element;
*/

//seive for prime generation
//using the seive the time complexity becomes n(log(log(n)))  
/*		int x;
	cin>>x;

	int n=floor(sqrt(x))+7;
	vector<int> seive(n , 1);
	
	seive[0]=0;seive[1]=0;


	for(int i=2;(i*i)<n;i++){
		cout<<i<<'\n';
		cout<<seive[i]<<'\n';
		if(seive[i]==1){
			for(int j=(i*i);j<n;j+=i){
				seive[j]=0;

			}
		}
	}
*/

/*
//binary search
  
  //in a sorted array (inc:a[i]<=a[i+1]) 2 options
  //max(i): , a[i]<=x  , from left bound (lower bound)
  //min(i): , a[i]>=x , from right bound(upper bound)

  int l=-1;int r=n;
  while(r>l+1){
	int mid=l+(r-l)/2;

// left case : invariant a[l]<=x , a[r]>x
	if(a[mid]<=x){
		l=mid;
	}
	else{
	    r=mid;
	}

// right case: invariant a[l]<x , a[r]>=x
	if(a[mid]>=x){
		r=mid;
	}
	else{
		l=mid;
	}


  }

*/

/*
to prevent precision based issues in ld comparision:
diff>=0.000000000001
--> not equal
*/

int mod=1000000007;

typedef long long ll;
typedef long double ld;

void solve(){

	int n;
	cin>>n;

	vector<ll> h(n , 0);
	for(int i=0;i<n;i++){
		cin>>h[i];
	}


	//ODD CASE IS TRIVIAL
	ll ans=0LL;
	if( (n)%2 != 0 ){
		for(int i=1;i<(n-1);i+=2){
			ll x=0LL;ll y=0LL;
			if( h[i] <= h[i+1] ){x+= ( h[i+1]-h[i]+1 );}
			if( h[i] <= h[i-1] ){y+= ( h[i-1]-h[i]+1);}
			ans+=max(x , y);
		}
		cout<<ans<<'\n';return;
	}



	//now we will have to deal with n = even case
	vector<int> i1;
	vector<int> i2;

	for(int i=1;i<(n-1);i+=2){i1.push_back(i);
		//cout<<i<<" ";
	}
	//cout<<endl;
	for(int i=2;i<(n-1);i+=2){i2.push_back(i);
		//cout<<i<<" ";
	}
		//cout<<endl;
//cout<<i1.size()<<endl;;
	ans=LLONG_MAX;ll curr_ans=0LL;
	
	ll prefix=0LL; //i1
	ll suffix=0LL; //i2

	for(int i=0;i<i2.size();i++){
		int ind=i2[i];
		ll x=0LL;ll y=0LL;
		if( h[ind] <= h[ind+1] ){x+= ( h[ind+1]-h[ind]+1 );}
		if( h[ind] <= h[ind-1] ){y+= ( h[ind-1]-h[ind]+1);}
		suffix+=max(x , y);

		//curr_ans+=(suffix + prefix);
	}
	curr_ans+=(suffix + prefix);
	ans=min(ans , curr_ans);

	

	for( int i=0; i < i1.size(); i++ ){

		//cout<<curr_ans<<endl;

		//i : represents the index till which u take i1
		//j : represents the index till which you take i2
		//cout<<"here";
		//cout<<curr_ans<<endl;
		curr_ans=0LL;
		//cout<<" curr+curr_ans " << " " << curr_ans<<endl;
		//first check for i1

		//cout<<"suffix"<<" "<<suffix<<" "<<"prefix"<<" "<<prefix<<'\n';

		int ind1=i1[i];
		int ind2=i2[i];

		// i1 / prefix
		ll x=0LL;ll y=0LL;
	
		if( h[ind1] <= h[ind1+1] ){x+= ( h[ind1+1]-h[ind1]+1 );}
		if( h[ind1] <= h[ind1-1] ){y+= ( h[ind1-1]-h[ind1]+1);}
		prefix+=max(x , y);
		
		//i2 / suffix
		x=0LL;y=0LL;
		
		if( h[ind2] <= h[ind2+1] ){x+= ( h[ind2+1]-h[ind2]+1 );}
		if( h[ind2] <= h[ind2-1] ){y+= ( h[ind2-1]-h[ind2]+1);}
		suffix-=max(x , y);
	
			
		curr_ans += (suffix + prefix);

	

		ans=min(ans , curr_ans);
	}

	cout<<ans<<'\n';


	return;
}

int main(){
ios_base::sync_with_stdio(false);
    cin.tie(NULL);
	int t;
	cin>>t;

	while(t--){
		solve();
	}

	return 0;
}