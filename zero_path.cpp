...
void solve(){
	...
	//we store dp as max, min

	for(int i=1;i<=n;i++){
		for(int j=1;j<=m;j++){

			if( i==1 && j==1 ){dp[i][j].first=a[i][j];dp[i][j].second=a[i][j];}

			if( (i-1) < 1){
				dp[i][j].first=a[i][j] + dp[i][j-1].first;
				dp[i][j].second=a[i][j] + dp[i][j-1].second;

			}
			else if( (j-1) < 1 ){
				dp[i][j].first=a[i][j] + dp[i-1][j].first;
				dp[i][j].second=a[i][j] + dp[i-1][j].second;

			}
			else{
				dp[i][j].first=a[i][j] + max(dp[i-1][j].first , dp[i][j-1].first);
				dp[i][j].second=a[i][j] + min(dp[i-1][j].second , dp[i][j-1].second);
