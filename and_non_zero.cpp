...
void solve(){
	int col_size=round(2*pow(10 , 5))+1;
	vector<vector<int>> prefix(19 , vector<int>(col_size , 0));
	...
	while(t--){
		...
		for(int i=0;i<=18;i++){
			int remove=prefix[i][r]-prefix[i][l-1];
			min_remove=min(min_remove , (size-remove));
		}
