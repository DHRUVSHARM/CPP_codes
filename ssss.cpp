...
void solve(){
	...
	//only primes till the sqrt(x) ie; till n are required
	//for computing factors 
	vector<int> primelist;
	for(int i=0;i<n;i++){
	...
	cout<<"prime list for computation"<<endl;
	for(auto x:primelist){
		cout<<x<<" ";
	}
	cout<<endl;
	//now we will compute the prime factors
	vector<int> factors;
