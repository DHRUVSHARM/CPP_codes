...
void solve(){
	...
	//we will check all possible combinations for every element
	//225 for each element

	for( int i=0;i<n;i++ ){
		ll min_operations = 15;

		for( ll cnt_add = 0 ; cnt_add <= 15 ; cnt_add++ ){
			for( ll cnt_mul = 0; cnt_mul <=15;  cnt_mul++ ){
				if( ((a[i] + cnt_add) * llround(pow(2 , cnt_mul))) % 32768 == 0 ){
					//possible combination of an answer
					min_operations = min( min_operations , (cnt_mul + cnt_add) );
				}
			}
		}

		//found minimal stuff
		ans.push_back(min_operations);
	}
