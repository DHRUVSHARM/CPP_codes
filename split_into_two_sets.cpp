...
int dfs(int vertex){
	//this dfs returns the length of cycle formed from vertex
	visited[vertex]=true;
	for(auto ind : graph[vertex]){
		if(!visited[ind]){
			//performing dfs so need to visit one of the 2 neighbours enough
			return dfs(ind) + 1;
		}
	}

	//this is the end so return 1
	return 1;
}
