...
void solve(){
	...
	ll l_cost=0LL;ll r_cost=0LL;
	ll lr_cost=0LL;int max_len=0;
	...
	for(int s=1;s<=n;s++){
		...
		//maintain the minimal l with lowest cost
		if( l < L ){
			L=l;
			l_cost=c;
		}
		...
		//maintain the maximal r with lowest cost
		if(r > R){
			R=r;
			r_cost=c;
		}
		...
		//maintain the maximal sized segment with minimal cost
		if( (r-l+1) > max_len ){
			max_len=(r-l+1);
			lr_cost=c;
		}
		else if( (r-l+1) == max_len ){
			lr_cost=min(lr_cost , c);
		}

		ll ans=l_cost + r_cost;
		if(max_len >= (R-L+1)){
			ans=min(ans , lr_cost);
		}
