...
void solve(){
	...
	if(even.size()==0 && x%2==0){cout<<"No"<<'\n';return;}

	int odd_selcted=0;
	odd_selcted=odd.size();
	if(odd.size()%2==0){odd_selcted--;}

	//cout<<"odd_selcted"<<" "<<odd_selcted<<endl;

	if(odd_selcted>=x){cout<<"Yes"<<endl;return;}
	x-=odd_selcted;

	int even_selected=even.size();
	if(even_selected>=x){cout<<"Yes"<<endl;return;}
	cout<<"No"<<endl;
	return;
}
