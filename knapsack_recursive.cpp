...
void helper( vector<pair<ll,ll>>& elements , int index 
	, ll curr , ll& maximum_value , ll W  ){

	//base cases
	if( index == elements.size()){
		//have to make a decision
		//if you have reached here means this is a valid path to
		//a solution, wrong combinations are terminated midway
		//cout << "possibility : "  << curr << endl;
		maximum_value = max( maximum_value , curr );
	...
	if( W - elements[index].first >= 0 ){
		//in limits , can add
		helper( elements , index + 1 , curr + elements[index].second 
			, maximum_value , W - elements[index].first  );
	}

	//second is we exclude, that requires no pre-check
	helper( elements , index + 1 , curr , maximum_value , W );
...
void solve(){
	...
	vector<pair<ll , ll>> elements(n , {0LL , 0LL});
	...
	ll maximum_value = 0LL;

	//cout << "W is : " << W << endl;

	helper( elements , 0 , 0 , maximum_value , W );
