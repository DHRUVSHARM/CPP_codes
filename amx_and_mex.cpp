...
void solve(){
	...
	//these are n distinct integers
	//first we will find mex and max
	int mex=0;int maxi=-1;
	sort(a.begin() , a.end());
	for(int i=0;i<n;i++){
		if(mex == a[i]){mex++;}
		maxi=max(maxi , a[i]);
	...
	if( mex > maxi ){cout<<(n+k)<<'\n';}
		...
		int ele_add= ceil(( maxi + mex )/2.0);
		//cout<<ele_add<<endl;
		if( find( a.begin() , a.end() ,  ele_add) == a.end() ){cout<<(n+1)<<'\n';}
		else{cout<<n<<'\n';}
	}
