...
void helper( vector<vector<ll>>& a , int n , int m , ll curr , 
ll& maximum_path_sum , int row , int column ){

	//we need to explore all paths that end up on destination
	if( row == n || column == m ){
		//we are out of range and will never reach the target
		//we stop exploring this path right here
		return;
	}

	//now we have reached the target, so we must consider this solution
	if( row == (n-1) && column == (m-1) ){
		maximum_path_sum = max(maximum_path_sum , curr + a[row][column]);
		return;
	}

	//we have 2 places to go

	helper( a , n , m ,  curr + a[row][column] , maximum_path_sum , 
	row + 1 , column );
...
void solve(){
	...
	ll maximum_path_sum = LLONG_MIN;

	helper( a , n , m , 0LL , maximum_path_sum , 0 , 0 );

	cout << maximum_path_sum << '\n';
