...
void solve(){
	...
	ll ans=0LL;
	vector<vector<ll>> frequency(12 , vector<ll>(12 , 0LL));
	//this will store the frequency of each 2 letter string
	...
	for( int i=0; i<n ; i++ ){
		...
		for( char c='a'; c <= 'k' ; c++ ){
			...
			for(int j=0; j<2 ; j++){
				...
				string a = s;
				a[j]=c;
				//made a change in position j only
				ans = ans + frequency[a[0]-'a'][a[1]-'a'];

			}
		}

		frequency[s[0]-'a'][s[1]-'a']++;
	}
