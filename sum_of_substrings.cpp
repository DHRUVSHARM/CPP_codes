...
void solve(){
	...
	if(k!=0){
		//first we will move the ones to last if required 
		if(a[a.size()-1] != 1){
			int j=a.size()-1;
			while(j>=0 && a[j]==a[a.size()-1]){
				j--;
			}
			if(j!=-1){
				int swaps=a.size()-j-1;
				if(swaps <= k){k-=swaps;swap( a[j] , a[a.size()-1] );}
			}
		}
	}
	if(k!=0){
		if(a[0]!=1){
			...
			if(j<(n-1)){
				int swaps=j;
				if(swaps <= k){k-=swaps;swap( a[j] , a[0] );}
